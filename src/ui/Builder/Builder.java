...
public class Builder {
    ...
    public GridPane build(Object sIn, GridPane grid) {
        if (!(sIn instanceof TabMenu)) {
            System.out.println(
                    "ERROR: Builder tried to build an object that cannot be made into a menu: "
                            + sIn);
            return grid;
        }
            ...
            switch (menuItems.getType(i)) {
                ...
                case SLIDER:
                    ...
                    if (c.getDigitType() == Constraint.Type.DOUBLE) {
                        ...
                        if (output.getValue() >= c.getMinDouble()
                                && output.getValue() <= c.getMaxDouble()) {
                        ...
                        digits =
                                String.valueOf(
                                                String.format(
                                                        "%." + dString + "f", c.getMaxDouble()))
                                        .length();
                        tf.setText(
                                String.format(
                                        "%." + dString + "f", s.valueProperty().floatValue()));
                        s.valueProperty()
                                .addListener(
                                        (observable, oldValue, newValue) -> {
                                            tf.setText(
                                                    String.format(
                                                            ...
                                                            newValue.floatValue()));
                                            output.setValue(
                                                    Math.round(newValue.floatValue() * (1 / dInc))
                                                            / (1 / dInc));
                                            tabController.setChanged(true);
                                            tabController.setValidity(client.check(), sIn);
                                        });
                        ...
                        if (output.getValue() >= c.getMinInt()
                                && output.getValue() <= c.getMaxInt()) {
                            s.setValue(output.getValue());
                        } else {
