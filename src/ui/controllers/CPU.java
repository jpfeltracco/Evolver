...
public class CPU implements Runnable {
    ...
    public CPU(
            ...
            AreaChart<Number, Number> cpuChart,
            AreaChart<Number, Number> threadChart,
            AreaChart<Number, Number> vDriverChart) {
            ...
            try {
                ...
                if (!list.isEmpty()) {
                    ...
                    if (value != -1.0) {
                        ...
                        addToSystemGraph(
                                "CPU Usage", cpuChart, timeSec, ((int) (value * 1000) / 10.0));
                ...
                addToSystemGraph(
                        "Peak Thread Count", threadChart, timeSec, thread.getPeakThreadCount());

                addToSystemGraph(
                        "Max Heap",
                        memoryChart,
                        timeSec,
                        (double) mem.getHeapMemoryUsage().getCommitted() / 1048576.0);
                addToSystemGraph(
                        "Used Heap",
                        memoryChart,
                        timeSec,
                        (double) mem.getHeapMemoryUsage().getUsed() / 1048576.0);

                addToSystemGraph(
                        "Number of VDrivers", vDriverChart, timeSec, VDriver.getNumVDrivers());
    ...
    public void addToSystemGraph(
            String series, AreaChart<Number, Number> chart, Number data1, Number data2) {
        Platform.runLater(
                new Runnable() {
                    @Override
                    public void run() {
                        Data<Number, Number> dat = new XYChart.Data<Number, Number>(data1, data2);
                        ObservableList<Data<Number, Number>> dataHolder =
                                getSeries(series, chart).getData();
                        dataHolder.add(dat);
                        ...
                        if (dataHolder.size() > MAXSIZE) {
                            ...
                            ((NumberAxis) chart.getXAxis())
                                    .setLowerBound((double) dataHolder.get(0).getXValue());
                        }
                    }
                });
    }
        ...
        switch (chart.getTitle()) {
            case "Memory":
                for (int i = 0; i < memorySeriesTitles.size(); i++) {
                    if (memorySeriesTitles.get(i).equals(name)) return chart.getData().get(i);
                }
                break;
            case "CPU":
                for (int i = 0; i < cpuSeriesTitles.size(); i++) {
                    if (cpuSeriesTitles.get(i).equals(name)) return chart.getData().get(i);
                }
                break;
            case "Threads":
                for (int i = 0; i < threadSeriesTitles.size(); i++) {
                    if (threadSeriesTitles.get(i).equals(name)) return chart.getData().get(i);
                }
                break;
            case "Virtual Tab Data":
                for (int i = 0; i < vDriverSeriesTitles.size(); i++) {
                    if (vDriverSeriesTitles.get(i).equals(name)) return chart.getData().get(i);
                }
                break;
        }
