...
public class EATab {
    ...
    public EATab(
        ...
        if (elements == null) fitnessGrapher.setGeneration(0);
        else fitnessGrapher.setGeneration(elements.getGen());
                ...
                controllers[i]
                        .setConfig(
                                elementHolder.getElements()[
                                                elementHolder.getElements().length - 1 - i]
                                        .clone());
            }
    ...
    public void saveFitnessGraphData() {
        if (graphSelector.getSelectionModel().getSelectedIndex() == 0) saveChartAsXML(fitnessGraph);
    ...
    protected void saveFitnessGraph() {
        if (graphSelector.getSelectionModel().getSelectedIndex() == 0) saveChartAsPng(fitnessGraph);
        else if (graphSelector.getSelectionModel().getSelectedIndex() == 1)
    ...
    public void initializeTab() {
                    ...
                    public void handle(Event t) {
                        if (close()) {
                            if (!tab.getId().contains("FILE"))
                                fxController.EATabsArray.set(
                                        fxController.EATabsArray.indexOf(t.getSource()), null);
        ...
        try {
            ...
            Parent root =
                    (Parent) fxmlLoader.load(this.getClass().getResource(FXMLTYPE).openStream());
            tab.setContent(root);
    ...
    public boolean saveAll(
            boolean saveElements, File selectedDirectory, SaveObject output, int index) {
        ElementHolder elementHolder = null;
        if (saveElements) elementHolder = getElementHolder();
        try {
            if (saveElements) output.graph[index] = fitnessGrapher.streamData();
        ...
        String name =
                selectedDirectory.getName().substring(0, selectedDirectory.getName().indexOf("."));
    ...
    public void setTabText(String name) {
        tabText = name;
        if (!original) return;
        if (changed) tab.setText("*" + tabText);
        else tab.setText(tabText);
        updateName();
    }
    ...
    public void saveChartAsPng(LineChart<Number, Number> graph) {
        ...
        if (GUI.lastFileName != null
                && GUI.lastFileName.length() > 0
                && GUI.checkExt(GUI.lastFileName.substring(GUI.lastFileName.indexOf(".")), exts))
    ...
    @FXML public TetheredButton startButton;

    @FXML protected Button clearButton;

    @FXML protected Button renderButton;

    @FXML protected ComboBox<String> simType;

    @FXML protected ComboBox<String> controllerType;

    @FXML protected LineChart<Number, Number> fitnessGraph;

    @FXML protected LineChart<Number, Number> actionGraph;

    @FXML protected TitledPane simPane;

    @FXML protected ScrollPane simScrollPane;

    @FXML protected TitledPane controllerPane;

    @FXML protected ScrollPane controllerScrollPane;

    @FXML protected TitledPane evolutionPane;

    @FXML protected ScrollPane evolutionScrollPane;

    @FXML public CheckBox simulationValid;

    @FXML public CheckBox controllerValid;

    @FXML public CheckBox evolutionValid;

    @FXML protected Button graphClean;

    @FXML public Label simLabel;

    @FXML public Label controlLabel;

    @FXML public Accordion accordion;

    @FXML protected Label generationTag;

    @FXML protected Label fitnessTag;

    @FXML protected StackPane SimComboBox;

    @FXML public TetheredProgressBar progressBar;

    @FXML public Label progressLabel;

    @FXML public TabPane graphSelector;
}
