...
public class DataBridge {
    ...
    public synchronized void graphData(String series, ChartType t, Number[] data) {
        ...
        } else addToSeries(series, t, data);
    }
    ...
    public synchronized void simplifyData(String seriesName, ChartType t, int averageFrames) {
        ...
        ObservableList<XYChart.Data<Number, Number>> seriesData =
                getSeries(seriesName, t).getData();
        ObservableList<XYChart.Data<Number, Number>> newSeriesData =
                FXCollections.observableArrayList();

        for (int i = 0; i < seriesData.size(); i += averageFrames) {
            double avg = 0;
            if (i + averageFrames > seriesData.size()) break;
            for (int j = 0; j < averageFrames; j++) {
                avg += seriesData.get(i + j).getYValue().doubleValue();
            }
            newSeriesData.add(
                    new XYChart.Data<Number, Number>(
                            seriesData.get(i + averageFrames - 1).getXValue().intValue(),
                            avg / ((double) averageFrames)));
        }
    ...
    public int getGraphSize(String name, ChartType t) {
        if (getSeries(name, t) != null) return getSeries(name, t).getData().size();
        return 0;
    }
        ...
        if (!virtual) {
            Platform.runLater(
                    new Runnable() {
                        @Override
                        public void run() {
                            eaTab.progressBar.setTetherProgress(
                                    d, generateTimeRemainingLabel(d, timeAtStart));
    ...
    public void setGeneration(int g) {
        if (!virtual) {
            if (lastGenUpdate == -1) lastGenUpdate = System.nanoTime();
    ...
    public void setFitness(double d) {
        if (!virtual) {
            if (lastFitnessUpdate == -1) lastFitnessUpdate = System.nanoTime();
    ...
    private String generateTimeRemainingLabel(double progressPercentage, long timeAtStart) {
        if (timeAtStart == -1) return "--";
        long elapsedTime = System.nanoTime() - timeAtStart;
        double secRemaining =
                (double) elapsedTime / progressPercentage / 1000000000.0
